...
class SimulationRunner:
    ...
    def __init__(self, ep_dir: Optional[str] = None, strict_mode: bool = True):
        """
        ...
        Args:
            ...
            strict_mode: If True, raises RuntimeError if EnergyPlus not found.
                        Only set to False for testing with explicit --test flag.
        """
        # EnergyPlus paths
        if ep_dir is None:
        ...
        # HARD CHECK: Verify EnergyPlus is installed
        if not os.path.exists(self.ep_exe):
            if strict_mode:
                raise RuntimeError(
                    f"EnergyPlus not found at {self.ep_exe}. "
                    "This skill requires actual EnergyPlus simulations for compliance analysis. "
                    "Mock data is not permitted. Install EnergyPlus with: bash lib/install.sh"
                )
            else:
                logger.warning(f"EnergyPlus not found at {self.ep_exe} (test mode)")
    ...
    def run_simulation(
        ...
        try:
            ...
            return {
                "success": True,
                "source": "energyplus",  # Provenance tag - required for artifact validation
    ...
    def parse_sql_results(self, sql_path: str, sim_dir: str) -> Dict[str, Any]:
        ...
        # FIX Bug B: Add provenance tag (artifact_builder requires this)
        result = {
            'source': 'energyplus',
            'eui_kwh_per_sf': None,
        ...
        try:
            ...
            # FIX Bug 7: Filter to annual report only (not monthly)
            cursor.execute("""
            ...
            # FIX Bug 6: Initialize accumulators for end-uses (sum across fuel types)
            end_use_totals_gj = {key: 0.0 for key in end_use_map.values()}

            # Parse rows
            for table_name, column_name, row_name, units, value in rows:
                try:
                    # FIX Bug B: Total Site Energy (must filter by column_name to avoid intensity rows)
                    # Without the column_name guard, we read 'Energy Per Conditioned Building Area' (MJ/m²)
                    # instead of 'Total Energy' (GJ), causing 5× EUI underestimate
                    if table_name == 'Site and Source Energy' and row_name == 'Total Site Energy' and column_name == 'Total Energy':
                    ...
                    # FIX Bug 6: Accumulate end-uses across all fuel columns
                    elif table_name == 'End Uses' and row_name in end_use_map:
                        energy_gj = float(value)
                        key = end_use_map[row_name]
                        end_use_totals_gj[key] += energy_gj  # Sum across fuel types
            ...
            # Convert accumulated end-uses to result dicts
            for key, total_gj in end_use_totals_gj.items():
                result['end_uses'][key] = total_gj
                result['end_uses_kwh'][key] = round(total_gj * 277.778, 0)
            ...
            # Add provenance tag to parsed results
            result['source'] = 'energyplus'
    ...
    def run_with_retry(
        ...
        for attempt in range(max_retries + 1):
            ...
            if result.get('success'):
                ...
                # Ensure provenance tag is present
